...
const unsigned long writeChannelID = 1902281;
const char *writeAPIKey = "BT4CUUI80PXSGMG3";

String cse_ip = "192.168.164.219"; // YOUR IP from ipconfig/ifconfig
...
{
  for (unsigned int i = CO2; i < ArrSize; i++)
  {
    String fieldData = "field" + String(i) + "=" + String(sensorVal[i - 1]);
...
void OM2MWrite()
{
  ...
  {
    HTTPClient http;
    http.begin(server + ae + "/" + String(cnt[i - 1]) + "/");
    http.addHeader("X-M2M-Origin", "admin:admin");
    http.addHeader("Content-Type", "application/json;ty=4");

    int val = http.POST("{\"m2m:cin\": {\"cnf\":\"application/json\",\"con\": " + String(sensorVal[i]) + "}}");
